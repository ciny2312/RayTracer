...
impl Vec3{
    ...
    fn cross(x:&Self,y:&Self)->Self{
        Self{
            e:[x.e[1]*y.e[2]-x.e[2]*y.e[1],x.e[2]*y.e[0]-x.e[0]*y.e[2],x.e[0]*y.e[1]-x.e[1]*y.e[0]],
        }
    }
...
fn ray_color(r:&Ray) -> Color{
    ...
    return Vec3::add(&Vec3::mul(&Color{e:[1.0,1.0,1.0]},1.0-a),&Vec3::mul(&Color{e:[0.5,0.7,1.0]},a));
}
fn main(){
    ...
    let viewport_upleft=Vec3::del(&Vec3::del(&Vec3::del(&camera_center,&Vec3{e:[0.0,0.0,-focal_length]})
    ,&Vec3::div(&viewport_u,2.0)),&Vec3::div(&viewport_v,2.0));
    let pixel_loc=Vec3::add(&viewport_upleft,&Vec3::div(&Vec3::add(&delta_u,&delta_v),2.0));

    writeln!(file, "P3\n{} {}\n255", width, height).unwrap();
    for j in 0..height{
        for i in 0..width{
            let pixel_center=Vec3::add(&pixel_loc,&Vec3::add(&Vec3::mul(&delta_u,i as f64),&Vec3::mul(&delta_v,j as f64)));
